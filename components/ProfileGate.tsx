  ...
  const form = (
    <div className="grid gap-4">
      <label className="grid gap-2 text-sm font-medium">
        昵称
        <input className="focus-ring rounded-md border border-ink/15 bg-white px-3 py-2" value={nickname} onChange={(event) => setNickname(event.target.value)} maxLength={24} />
      </label>
      <div className="grid gap-2">
        ...
        <div className="flex flex-wrap gap-2">
          {avatars.map((item) => (
            <button
              className={`focus-ring flex items-center gap-2 rounded-md border px-3 py-2 text-sm ${avatar === item.id ? "border-clay bg-clay/10" : "border-ink/10 bg-white"}`}
              key={item.id}
      ...
      {!profile && (
        <label className="grid gap-2 text-sm font-medium">
          提交码
          <input className="focus-ring rounded-md border border-ink/15 bg-white px-3 py-2" value={editCode} onChange={(event) => setEditCode(cleanEditCode(event.target.value))} maxLength={8} />
        </label>
      )}
      {profile && editing && (
        <label className="grid gap-2 text-sm font-medium">
          输入提交码验证修改
          <input className="focus-ring rounded-md border border-ink/15 bg-white px-3 py-2" value={verifyCode} onChange={(event) => setVerifyCode(cleanEditCode(event.target.value))} maxLength={8} />
        </label>
      )}
      {message && <p className="text-sm text-clay">{message}</p>}
      <button className="focus-ring rounded-md bg-ink px-4 py-2 text-sm font-semibold text-white disabled:opacity-50" disabled={busy} onClick={profile ? updateProfile : createProfile} type="button">
  ...
  if (!profile) {
    return (
      <section className="mx-auto max-w-xl rounded-lg border border-ink/10 bg-white p-6 shadow-soft">
        <h2 className="text-xl font-semibold">先给自己一个轻账户</h2>
  ...
  return (
    <div className="rounded-lg border border-ink/10 bg-white p-4">
      <div className="flex items-center justify-between gap-3">
        ...
        <button className="focus-ring rounded-md border border-ink/15 px-3 py-2 text-sm" onClick={() => setEditing((value) => !value)} type="button">
          修改资料
        </button>
      </div>
